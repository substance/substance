...
test("convertContainer() -- trailing text", (t) => {
  ...
  t.equal(container.nodes.length, 2, 'should have two nodes')
...
test("resolving id collisions", (t) => {
  ...
  let container = importer.convertContainer(els, 'body')
  t.equal(container.nodes.length, 2, 'should have two nodes')
  t.equal(container.nodes[0], 'foo', 'first should have the original id')
  t.notEqual(container.nodes[1], 'foo', 'second should have a new one')
  t.end()
})
